...
function FilterSort() {
    ...
    function handleClick() {
        let filtered = [];
        breeds.forEach((b) => {
            b.temperaments.map(t => t.name === selectedTemp ?
                filtered.push(b) : null
            )
        })
    ...
    return (
        <div className={style.mainContainer}>
            <form className={style.formContainer} onSubmit={handleSubmit}>
                <select onChange={handleChangeTemp} name="temperaments" value={selectedTemp} className={style.select}>
                    <option>
                        Filter Temperament
                    </option>
                    {temperaments.map((e) => (
                ...
                <div className={style.btnContainer}>
                    <button onClick={resetTemps} className={style.button} >Clear</button>
                    <button onClick={() => handleClick()} className={style.button}>Filter</button>
                </div>
            </form>
